...
function cd_crt_count_bot()
{
    ...
    if( preg_match( '#(' . implode( '|', $bots ) . ')#i', $agent, $matches ) )
    {
        ...
        $data['crawl_date'] = gmdate( 'Y-m-d', time() + get_option('gmt_offset')*3600 );
        $data['crawl_time'] = gmdate( 'H:i:s', time() + get_option('gmt_offset')*3600 );
